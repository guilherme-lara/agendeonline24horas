...
const Aniversarios = () => {
  ...
  const { data: customers = [], isLoading, isError, refetch } = useQuery({
    queryKey: ["birthdays", clinic?.id],
    queryFn: async () => {
      if (!clinic?.id) return [];
      const { data, error } = await supabase.from("customers").select("id, name, phone, birth_date").eq("barbershop_id", clinic.id).not("birth_date", "is", null);
  ...
  const sendWhatsApp = (phone: string, name: string) => {
    if (!phone) return;
    const msg = encodeURIComponent(`🎁 Parabéns, ${name}! Feliz Aniversário! 🎉 Como presente, você tem 10% de desconto no seu próximo agendamento. Agende já: ${window.location.origin}/agendamentos/${clinic?.slug}`);
