...
export function SalesList({ barbershopId, status, onSelectSale, createdBy }: SalesListProps) {
  ...
  if (!sales || sales.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center text-center p-8 bg-zinc-50 dark:bg-zinc-900 border border-dashed rounded-2xl">
        <Receipt className="w-10 h-10 text-zinc-300 dark:text-zinc-700 mb-2" />
  ...
  return (
    <div className="flex flex-col gap-2 p-2">
      {sales.map((sale: any) => (
        <div key={sale.id} className="bg-card border rounded-xl p-4 flex items-center justify-between hover:shadow-xs transition-shadow">
          <div>
            ...
            <p className="text-xs font-semibold text-muted-foreground uppercase">
              {sale.sales_items?.length || 0} itens • R$ {Number(sale.total_amount).toFixed(2).replace(".", ",")}
            </p>
          </div>
