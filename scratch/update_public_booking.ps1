...
# 2. Update bookingMutation to consider paymentOption
...
# 3. Add UI radio buttons in step 4 and update button text
...
# 4. Success Screen Update
$findSuccess = '(?s)Agendamento Confirmado!</h1>\s*<p className="text-muted-foreground mb-8 max-w-xs mx-auto">Seu pagamento foi aprovado. A sua vaga está garantida e te esperamos no horário marcado.</p>'
$replaceSuccess = @'
Agendamento Finalizado!</h1>
                ...
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-6 mb-8 text-left">
                  ...
                  <Button
                    onClick={() => {
                      const msg = encodeURIComponent(`Olá, gostaria de confirmar meu agendamento para o dia ${format(selectedDate, 'dd/MM/yyyy')} às ${selectedTime} com ${selectedBarber?.name}.`);
                ...
                <p className="text-muted-foreground mb-6 max-w-xs mx-auto">Te esperamos no dia {format(selectedDate, "dd 'de' MMMM", { locale: ptBR })} às {selectedTime}.</p>
'@
